...
pub fn register(ctx: &Ctx<'_>) -> crate::Result<()> {
    ...
    let set_timeout = Function::new(
        ctx.clone(),
        |callback: rquickjs::Function<'_>, _ms: Option<i32>| -> i32 {
    ...
    let set_interval = Function::new(
        ctx.clone(),
        |_callback: rquickjs::Function<'_>, _ms: Option<i32>| -> i32 {
    ...
    let clear_timeout = Function::new(ctx.clone(), |_id: i32| {}).map_err(|e| {
        crate::JsRaftError::Extension(format!("Failed to create clearTimeout: {e}"))
    })?;
    ...
    let clear_interval = Function::new(ctx.clone(), |_id: i32| {}).map_err(|e| {
        crate::JsRaftError::Extension(format!("Failed to create clearInterval: {e}"))
    })?;

    globals
        .set("clearInterval", clear_interval)
        .map_err(|e| crate::JsRaftError::Extension(format!("Failed to set clearInterval: {e}")))?;
