...
pub fn register(ctx: &Ctx<'_>) -> crate::Result<()> {
    ...
    let join = Function::new(ctx.clone(), |args_str: String| -> String {
        // Parse JSON array of segments
        let segments: Vec<String> = serde_json::from_str(&args_str).unwrap_or_default();
    ...
    let resolve = Function::new(ctx.clone(), |args_str: String| -> String {
        let segments: Vec<String> = serde_json::from_str(&args_str).unwrap_or_default();
    ...
    let relative = Function::new(ctx.clone(), |from: String, to: String| -> String {
